...
fig.suptitle('%s' %sys.argv[3], fontsize=20)

# read PUMAS file for values
data = pd.read_csv('%s.csv' %sys.argv[1])
...
for key in state_codes.keys()[0:int(sys.argv[2])]:
    m.readshapefile('../shapefiles/pums/tl_2013_{0}_puma10'.format(key),
